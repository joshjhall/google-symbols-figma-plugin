 ...
 *    - category-mapping.json (icon → category map for validation)
...
async function generateCategories() {
  ...
  try {
    ...
    const allIconsJsonPath = path.join(__dirname, '..', 'src', 'lib', 'icons', 'all-icons-data.json');
    ...
    let existingCategories: Array<{name: string, count: number, firstIcon: string, lastIcon: string}> = [];
    ...
    if (fs.existsSync(categoryMappingPath) && fs.existsSync(categoriesSummaryPath)) {
      ...
      existingCategories = JSON.parse(fs.readFileSync(categoriesSummaryPath, 'utf-8'));
      hasExistingMapping = true;
      console.log('📋 Found existing category mapping - will preserve existing assignments');
    ...
    const categories: Array<{name: string, icons: string[], count: number, lastIconExclusive: string}> = [];

    if (hasExistingMapping) {
      // PRESERVE EXISTING ASSIGNMENTS - Never move icons!
      console.log('🔒 Preserving existing icon assignments (no icons will be moved)\n');

      // Identify new icons
      const newIcons = allIcons.filter(icon => !existingMapping[icon]);
      console.log(`🆕 Found ${newIcons.length} new icons to assign\n`);
      ...
      if (newIcons.length > 0) {
        console.log('📚 Assigning new icons alphabetically (like adding books to library shelves)...\n');
        ...
        for (const newIcon of newIcons) {
          ...
          console.log(`   ${newIcon} → ${assignedCategory.name} (now ${assignedCategory.count} icons, ${assignedCategory.firstIcon}...${assignedCategory.lastIcon})`);
      ...
      for (const categoryName of categoryNames) {
        ...
        const lastIcon = lastIconIndex + 1 < allIcons.length ? allIcons[lastIconIndex + 1] : '__END__';
      ...
      console.log('🆕 No existing mapping - creating fresh categories\n');
    ...
    // Display results
    console.log('\n=====================================');
    console.log(hasExistingMapping ? 'UPDATED SETS (existing assignments preserved)' : 'EVENLY DISTRIBUTED SETS');
    console.log('=====================================');
    ...
    console.log('=====================================');
    console.log('Baseline: Set 8 (148 icons × 504 variants = 74,592 components) @ ~21.5% memory');
    console.log('Note: Actual memory usage can vary ±20% between sessions and sets');
    ...
    fs.writeFileSync(categoriesSummaryPath, JSON.stringify(
      categories.map(c => ({
        name: c.name,
        count: c.count,
    ...
    console.log('✅ Summary saved to src/data/categories-summary.json');
